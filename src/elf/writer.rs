...
impl<'a> ElfWriter<'a> {
    ...
    /// ```
    pub fn new(data: &'a [u8], elf: &crate::elf::Elf) -> error::Result<Self> {
    ...
    fn modify_rpath(&mut self, new_rpath: &str, force_rpath: bool) -> error::Result<()> {
        ...
        let rpath_entry = entries
            .iter()
            .enumerate()
            .find(|(_, e)| e.d_tag == crate::elf::dynamic::DT_RPATH);
        let runpath_entry = entries
            .iter()
            .enumerate()
            .find(|(_, e)| e.d_tag == crate::elf::dynamic::DT_RUNPATH);
        ...
        let target_tag = if force_rpath {
            crate::elf::dynamic::DT_RPATH
        } else {
            crate::elf::dynamic::DT_RUNPATH
        };
        ...
        if let Some(idx) = entry_idx {
            ...
            let null_pos = entries
                .iter()
                .position(|e| e.d_tag == crate::elf::dynamic::DT_NULL);
            let new_entry = Dyn {
                d_tag: target_tag,
    ...
    pub fn rpath_to_runpath(&mut self) -> error::Result<()> {
        if let Some(ref mut entries) = self.dynamic_entries {
            for entry in entries.iter_mut() {
                if entry.d_tag == crate::elf::dynamic::DT_RPATH {
                    entry.d_tag = crate::elf::dynamic::DT_RUNPATH;
                }
            }
        }
        Ok(())
    ...
    pub fn runpath_to_rpath(&mut self) -> error::Result<()> {
        if let Some(ref mut entries) = self.dynamic_entries {
            for entry in entries.iter_mut() {
                if entry.d_tag == crate::elf::dynamic::DT_RUNPATH {
                    entry.d_tag = crate::elf::dynamic::DT_RPATH;
                }
            }
        }
        Ok(())
    ...
    fn write_with_slack(&self) -> error::Result<Vec<u8>> {
        ...
        if let Some(ref entries) = self.dynamic_entries {
            ...
            for entry in &mut updated_entries {
                if entry.d_tag == crate::elf::dynamic::DT_STRSZ {
        ...
        for (idx, section) in self.section_headers.iter().enumerate() {
            if section.sh_type == crate::elf::section_header::SHT_STRTAB
    ...
    fn write_with_append(&self) -> error::Result<Vec<u8>> {
        ...
        let (highest_vaddr, highest_file_end) = self
            .program_headers
            .iter()
            .filter(|p| p.p_type == crate::elf::program_header::PT_LOAD)
        ...
        let padding_size = new_file_offset - self.data.len();
        ...
        if let Some(ref entries) = self.dynamic_entries {
            ...
            for entry in &mut updated_entries {
                match entry.d_tag {
                    crate::elf::dynamic::DT_STRTAB => {
                        entry.d_val = new_dynstr_vaddr;
                    }
                    crate::elf::dynamic::DT_STRSZ => {
            ...
            let null_entry = Dyn {
                d_tag: crate::elf::dynamic::DT_NULL,
                d_val: 0,
            };
        ...
        for (i, phdr) in self.program_headers.iter().enumerate() {
            ...
            if phdr.p_type == crate::elf::program_header::PT_DYNAMIC {
        ...
        let new_pt_load = ProgramHeader {
            p_type: crate::elf::program_header::PT_LOAD,
            p_flags: crate::elf::program_header::PF_R | crate::elf::program_header::PF_W,
        ...
        for (i, section) in self.section_headers.iter().enumerate() {
            ...
            if section.sh_type == crate::elf::section_header::SHT_STRTAB
                && section.sh_offset as usize == self.dynstrtab_offset
                && section.sh_flags & crate::elf::section_header::SHF_ALLOC as u64 != 0
            {
            ...
            if section.sh_type == crate::elf::section_header::SHT_DYNAMIC {
    ...
    fn write_with_relocation(&self) -> error::Result<Vec<u8>> {
        ...
        note_start_offset = (phdr_end + 3) & !3;
        ...
        // This is from PREPEND_SIZE to where the first non-relocated code section starts.
        // The note section ends at note_orig_addr + note_orig_size = 0x400318 + 0x24 = 0x40033c
        // which maps to file offset 0x33c in the original.
        // After prepending, the new offset is PREPEND_SIZE + 0x33c = 0x233c.
        // We zero from PREPEND_SIZE to there.
        if note_orig_addr != 0 {
        ...
        // This marks the area as relocated/unused
        // Use the original dynstrtab_size, not the current (potentially modified) size
        ...
        for phdr in &self.program_headers {
            ...
            if phdr.p_type == crate::elf::program_header::PT_GNU_STACK {
                updated.p_offset = 0;
            } else {
            ...
            if phdr.p_type == crate::elf::program_header::PT_PHDR {
            ...
            if phdr.p_type == crate::elf::program_header::PT_NOTE
                && note_orig_addr != 0
            ...
            // Check if this is an executable LOAD segment that starts before the note
            // and the note is within its range
            if phdr.p_type == crate::elf::program_header::PT_LOAD
                && note_orig_addr != 0
                && phdr.p_vaddr <= note_orig_addr
                && note_orig_addr < phdr.p_vaddr + phdr.p_filesz
                && (phdr.p_flags & crate::elf::program_header::PF_X) != 0
            {
                // Find the section after .note.gnu.build-id (.init typically)
                // Calculate how much to shrink by finding where .init starts
        ...
        // 1. PT_PHDR first
        // 2. PT_GNU_STACK second (patchelf puts this early)
        // 3. PT_LOAD segments sorted by vaddr
        // 4. All other segments in their relative order
        ...
        // Like patchelf, we need to sort sections by address
        ...
        for (idx, section) in self.section_headers.iter().enumerate() {
            ...
            else if section.sh_type == crate::elf::section_header::SHT_STRTAB
                && section.sh_offset as usize == self.dynstrtab_offset
            {
                dynstr_idx = Some(idx);
            ...
            else if section.sh_type != crate::elf::section_header::SHT_NULL {
                if section.sh_flags & crate::elf::section_header::SHF_ALLOC as u64 != 0 {
        ...
        // Sort sections by address, keeping NULL first and non-ALLOC sections at the end
        // This mimics patchelf's behavior:
        // 1. NULL section first
        // 2. ALLOC sections sorted by address
        // 3. Non-ALLOC sections sorted by offset
        updated_sections.sort_by(|a, b| {
            let (_, sec_a) = a;
            let (_, sec_b) = b;

            // NULL always first
            if sec_a.sh_type == crate::elf::section_header::SHT_NULL {
                return std::cmp::Ordering::Less;
            }
            if sec_b.sh_type == crate::elf::section_header::SHT_NULL {
                return std::cmp::Ordering::Greater;
            }

            // Non-ALLOC sections go to the end
            let a_alloc = sec_a.sh_flags & crate::elf::section_header::SHF_ALLOC as u64 != 0;
            let b_alloc = sec_b.sh_flags & crate::elf::section_header::SHF_ALLOC as u64 != 0;
            if a_alloc && !b_alloc {
                return std::cmp::Ordering::Less;
            }
            if !a_alloc && b_alloc {
        ...
        // Find symtab section and update st_shndx fields
        for (old_idx, section) in self.section_headers.iter().enumerate() {
            if section.sh_type == crate::elf::section_header::SHT_SYMTAB {
                ...
                for i in 0..num_syms {
                    ...
                    if sym_offset + sym_size <= output.len() {
                        ...
                        // Update if it's a valid section index (not special like SHN_UNDEF, SHN_ABS, etc.)
        ...
        // The dynamic section itself has been shifted, so we need to write at the new offset
        if let Some(ref entries) = self.dynamic_entries {
            let new_dynstr_vaddr = new_vaddr_base + dynstr_start_offset as u64;

            let mut updated_entries = entries.clone();
            for entry in &mut updated_entries {
                if entry.d_tag == crate::elf::dynamic::DT_STRTAB {
                    entry.d_val = new_dynstr_vaddr;
                } else if entry.d_tag == crate::elf::dynamic::DT_STRSZ {
                    entry.d_val = dynstr_size as u64;
                }
            }
    ...
    ///
    /// This returns the amount of unused space between the end of the string table
    /// and the start of the next section. This is "true" slack space that exists
    /// within the file and can be used without extending the file.
    ///
    /// Returns 0 if there's no slack space (next section immediately follows).
    fn check_slack_space(&self) -> usize {
        ...
        // (we'd be appending to the end of the file)
        let next_offset = match next_section_offset {
            Some(offset) => offset,
            None => return 0, // No slack space, would need to append/relocate
        };
    ...
    fn write_dynamic_section(&self, output: &mut [u8], entries: &[Dyn]) -> error::Result<()> {
        ...
        for ph in &self.program_headers {
            if ph.p_type == crate::elf::program_header::PT_DYNAMIC {
        ...
        for i in entries.len()..self.original_dynamic_count {
            let pos = offset + (i * entry_size);
            if pos + entry_size > output.len() {
                break; // Don't write beyond file size
            }
            null_entry
                .clone()
                .try_into_ctx(&mut output[pos..], self.ctx)?;
    ...
    /// Lower values sort first. This matches patchelf's ordering:
    /// - PT_PHDR comes first (priority 0)
    /// - PT_GNU_STACK comes second (priority 1)
    /// - Everything else is sorted by virtual address (priority 2)
...
mod tests {
    ...
    fn test_writer_creation() {
        let data: Vec<u8> = include!("../../etc/crt1.rs");
        if let Ok(elf) = Elf::parse(&data) {
            let writer = ElfWriter::new(&data, &elf);
            assert!(writer.is_ok());
        }
    }
}
